...
import { computeFitTransform, type Box3Like } from "./modelFit.js";
...
/**
 * The regression this port exists for: furniture used to be scaled by width alone, so
 * height and depth landed wherever the source model's proportions put them. With models
 * from several sources that made sizes visibly inconsistent between items.
 */
describe("consistent sizing across mixed model sources", () => {
  const box = (x: number, y: number, z: number): Box3Like => ({
    min: { x: 0, y: 0, z: 0 },
    max: { x, y, z }
  });

  it("keeps every axis within the declared dimensions", () => {
    // A tall, narrow source model asked to be a wide, low sideboard.
    const fit = computeFitTransform(box(1, 4, 1), { w: 1.4, h: 0.5, d: 0.4 });
    expect(1 * fit.scale).toBeLessThanOrEqual(1.4 + 1e-9);
    expect(4 * fit.scale).toBeLessThanOrEqual(0.5 + 1e-9);
    expect(1 * fit.scale).toBeLessThanOrEqual(0.4 + 1e-9);
  });

  it("gives two differently-proportioned models the same declared height cap", () => {
    const dims = { w: 0.55, h: 0.95, d: 0.55 };
    const squat = computeFitTransform(box(2, 1, 2), dims);
    const lanky = computeFitTransform(box(0.5, 3, 0.5), dims);
    expect(1 * squat.scale).toBeLessThanOrEqual(dims.h + 1e-9);
    expect(3 * lanky.scale).toBeLessThanOrEqual(dims.h + 1e-9);
  });

  it("scales uniformly, so nothing is stretched out of proportion", () => {
    const fit = computeFitTransform(box(2, 1, 4), { w: 1, h: 1, d: 1 });
    // One number applied to all axes — aspect ratio is preserved by construction.
    expect(typeof fit.scale).toBe("number");
    expect(fit.scale).toBeCloseTo(0.25, 9); // limited by the 4-deep axis
  });

  it("still rests the model on the floor after fitting", () => {
    const fit = computeFitTransform(
      { min: { x: -1, y: 0.7, z: -1 }, max: { x: 1, y: 2.7, z: 1 } },
      { w: 1, h: 1, d: 1 }
    );
    // The lowest point maps to y = 0 regardless of where the model sat in its own space.
    expect(0.7 * fit.scale + fit.position[1]).toBeCloseTo(0, 9);
  });
});
