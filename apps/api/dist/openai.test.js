...
vi.mock('openai', () => ({
    default: class MockOpenAI {
        chat = { completions: { create: (...args) => createMock(...args) } };
    },
}));
...
describe('analyzeRoomPhoto', () => {
    ...
    it('throws RoomPhotoConfigError when no API key is configured', async () => {
        await expect(analyzeRoomPhoto(IMAGE, { apiKey: undefined, model: 'gpt-4o', mock: false })).rejects.toBeInstanceOf(RoomPhotoConfigError);
        expect(createMock).not.toHaveBeenCalled();
    });
    ...
    it('throws RoomPhotoUpstreamError when the model response is not valid JSON', async () => {
        ...
        await expect(analyzeRoomPhoto(IMAGE, { apiKey: 'sk-test', model: 'gpt-4o', mock: false })).rejects.toBeInstanceOf(RoomPhotoUpstreamError);
    });
    ...
    it("throws RoomPhotoUpstreamError when the model response doesn't match the schema", () => {
        ...
        return expect(analyzeRoomPhoto(IMAGE, { apiKey: 'sk-test', model: 'gpt-4o', mock: false })).rejects.toBeInstanceOf(RoomPhotoUpstreamError);
    });
