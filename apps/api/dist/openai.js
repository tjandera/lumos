...
import { RoomPhotoProposalSchema } from '@interior/core';
 ...
 * data any further. */
export async function analyzeRoomPhoto(imageDataUrl, deps) {
    if (deps.mock)
        return mockProposal();
    if (!deps.apiKey)
        throw new RoomPhotoConfigError('OPENAI_API_KEY is not configured on the server');
    ...
    if (!content)
        throw new RoomPhotoUpstreamError('The model returned an empty response');
    let parsed;
    try {
